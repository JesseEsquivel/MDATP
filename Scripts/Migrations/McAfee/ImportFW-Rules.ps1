...
ForEach ($Rule In $FirewallRules)
{
	...
	$RuleSplatHash = @{
        ...
        #RemoteAddress = $($Rule.'Remote Address') -replace ('(\s|,\s+|\s-\s|\sand\s|and\s|-\s|\s-|\sand)', ",") -split "," #<--move to all regex for values that must be passed as an array
        #RemoteAddress = $($Rule.'Remote Address') -replace ('(,\s,\s+|\s|,\s+|\s-\s|\sand\s|and\s|-\s|\s-|\sand)', ",") -split "," #<--move to all regex for values that must be passed as an array
        #RemoteAddress = $($Rule.'Remote Address') -replace ('(\s+-\s+|\s+,\s+,\s+|\s+,\s+|,\s+|\s-\s|\s+and\s+|and\s+|-\s+|\s+-|\sand)', ",") -split "," #<--move to all regex for values that must be passed as an array
        ...
        #RemotePort = ListToStringArray $rule.'Remote Port'.replace(" ","") # <--- There must be no space in this string or cmdlet will fail :(
