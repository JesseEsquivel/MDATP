...
try
{
    ...
    Copy-Item -Path $vdmpath -Destination "\\n005uxn-filea01.sof.socds.mil\mdatp$\wdav-update" -Force -Recurse | Out-Null
    Get-ChildItem "\\n005uxn-filea01.sof.socds.mil\mdatp$\wdav-update\x64" -Recurse | ForEach-Object {Remove-Item $_.FullName -Recurse -Force}
    If(!(Test-Path -Path "\\n005uxn-filea01.sof.socds.mil\mdatp$\wdav-update\x64"))
    {
        New-Item -ItemType Directory -Force -path "\\n005uxn-filea01.sof.socds.mil\mdatp$\wdav-update\x64" | Out-Null
    }
    Copy-Item -Path "$vdmpath\*" -Destination "\\n005uxn-filea01.sof.socds.mil\mdatp$\wdav-update\x64" -Force -Recurse | Out-Null
}
catch
{
    Write-Host "Failed to copy extracted files to share \\n005uxn-filea01.sof.socds.mil\mdatp$\: $($_.Exception.Message)" -ForegroundColor Red
    log-This $errorEvent "Failed to copy extracted files to share \\n005uxn-filea01.sof.socds.mil\mdatp$\: $($_.Exception.Message)"
    closescript 1
}

Write-Host "Success." -ForegroundColor Green
log-This $infoEvent "Successfully copied extracted files to share: \\n005uxn-filea01.sof.socds.mil\mdatp$\wdav-update\x64"
Write-Host

try
{
    Write-Host "Pruning Folders older than 7 days..."
    Prune-Folders "-7" "\\n005uxn-filea01.sof.socds.mil\mdatp$\wdav-update"
...
Write-Host "Success." -ForegroundColor Green
log-This $infoEvent "Successfully pruned folders older than 7 days in C:\Windows\wdav-update, and in \\n005uxn-filea01.sof.socds.mil\mdatp$\wdav-update"
Write-Host
